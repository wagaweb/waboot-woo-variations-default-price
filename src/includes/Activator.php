...
namespace WBWooVariationsDefaultPrice\includes;
 ...
 * @subpackage WBWooVariationsDefaultPrice/includes
 */
class Activator {
	public static function activate($options = []) {
		...
		try{
			if($options['check_wbf_install']){
				$wbf_path = \WBWooVariationsDefaultPrice\includes\get_wbf_path();
			}else{
				if(!\WBWooVariationsDefaultPrice\includes\pluginsframework_is_present()){
